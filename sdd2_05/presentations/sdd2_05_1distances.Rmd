---
title: 'Science des données II : cours 5a'
...
Jusqu'ici nous nous sommes concentrés sur l'étude d'un petit nombre de variables à la fois.

Mais que faire lorsque le jeu de données est volumineux et que nous ne savons pas par où commencer\ ?

\hfill

Dans la partie **analyse** de ce cours, nous aborderons l’\alert{analyse exploratoire des données} ou EAD. Elle permet d’**explorer** des gros jeux de données multivariés et d’y découvrir des **structures** ou des **associations** entre individus.


### Analyse exploratoire de données

L’\alert{analyse de données} ou \alert{multivariée} se subdivise en deux familles de techniques complémentaires\ :
...
> - Les méthodes d’**ordination** ou de **réduction de dimensionnalité**. Elles représentent les individus sur des graphes bi- ou tridimensionnels (ou dit, une représentation sur des **cartes**, par analogie à la géographie). La distance des individus entre eux est relative à leur degré de ressemblance ou de différence (on dit\ : **similarité** ou **dissimilarité**).
