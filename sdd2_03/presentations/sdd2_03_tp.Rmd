---
title: 'Science des données II : T.P. 3'
...
L’UR2PI et le CIRAD souhaite trouver un modèle pour estimer la hauteur d’un arbre sur base de sa circonférence à 1,50m du sol.
...
knitr::kable(eucalyptus[ c(1, 110, 210, 610, 1010, 1410), ],
  col.names = c("Code d'identification", "Hauteur", "Circonférence", "Bloc"),
  align = 'c',
  caption = "Quelques lignes du tableau de données eucalyptus")
```
...
ovo_glm <- glm(data = ovo1, prop ~ conc,
  family = binomial(link = "logit"), weights = nombre)
#summary(ovo_glm)

chart(ovo1, prop ~ conc) +
  geom_point() +
  geom_line(mapping = aes( x =  conc , y = predict(ovo_glm, type = "response"))) +
  labs(y = "Fraction d'ovocyte entrant en maturation",
       x = "Concentration en hypoxantine")
```
