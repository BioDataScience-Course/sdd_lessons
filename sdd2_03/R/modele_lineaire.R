...
# Transformation voir plus haut ; garder aussi la variable 'gestation' en jours
# et 'ht', la taille de la mère en pouces à convertir en m (/ 39.37)
babies %>.%
  select(., gestation, smoke, wt1, ht) %>.%
...
boxplot(data = Babies_prem, bmi ~ premat)

# Modèle linéaire
lm(data = Babies_prem, premat ~ smoke + bmi)
...
res <- glm(data = Babies_prem, premat ~ smoke + bmi,
...
summary(Babies_prem)
res <- glm(data = Babies_prem, premat ~ smokeB + bmi,
  family = binomial(link = logit))
summary(res)
