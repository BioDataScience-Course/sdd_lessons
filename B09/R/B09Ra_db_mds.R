...
# version 2023-2024, 2024-03-12
# Ph. Grosjean, licence CC BY-NC-SA 4.0
#
...
# Attention : deux façons de sommer les cas où il n'y a que des valeurs
# manquantes `NA` par pays et par année :

sum(NA, na.rm = TRUE)
fsum(NA, na.rm = TRUE)

# Même calcul, mais en utilisant fsum() au lieu de sum()
...
# Pour éviter les relations cycliques qui vont poser problème avec dm_filter() :
dm_from_con(con, learn_keys = FALSE) %>.%
  dm_set_colors(., red = who, darkgreen = pop, blue = countries) %>.% # Couleurs (optionel)
  dm_add_pk(., pop, c(country, year)) %>.% # Clé primaire pop
  dm_add_pk(., who, c(country, year, method, sex, age)) %>.% # Clé primaire who
