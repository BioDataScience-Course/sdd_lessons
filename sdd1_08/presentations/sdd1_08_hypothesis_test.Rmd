---
title: 'Science des données I : module 78'
...
- $H_0$, l'hypothèse nulle =>  l'affirmation à réfuter
- $H_1$, l'hypothèse alternative

### Distribution du chi^2^

Cette distribution admet un paramètre qui sont les degrés de liberté (valeur de 3 pour la distribution ci-dessous)
...
Cette zone de rejet est déterminée par une probabilité nommé le seuil $\alpha$ déterminé avant de réaliser un test d'hypothèse.

Nous fixons le seuil alpha à 5% 
...
```{r}
(tab1 <- as.table(c(A = 5, B = 10, C = 20, D = 10)))
```

```{r}
chisq.test(tab1, p = c(1/4, 1/4, 1/4, 1/4))
```
